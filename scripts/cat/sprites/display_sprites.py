...
def generate_sprite(
    ...
    n = (list(constants.SPECIES["species"]).index(cat.species)) + 1 #add 1 because people don't count from 0

    poses: list = getattr(sprites, f"POSE_DATA_{n}")["poses"]
    ...
    try:
        if cat.pelt.name not in ["Tortie", "Calico"]:
            ...
            # Base Coat
            sprite_name = f"colours_{cat.pelt.tortie_base}{n}_{cat.pelt.colour}{cat_sprite}"
            new_sprite.blit(
            ...
            sprite_name = (
                f"colours_{tortie_pattern}{n}_{cat.pelt.tortie_colour}{cat_sprite}"
            )
            patches = sprites.sprites[sprite_name].copy()
            sprite_name = f"{getattr(sprites, f"TORTIE_DATA_{n}")['spritesheet']}{n}_{cat.pelt.tortie_marking}{cat_sprite}"
            patches.blit(
                sprites.sprites[sprite_name],
                (0, 0),
        ...
        if (
            ...
            and cat.pelt.tint in getattr(sprites, f"cat_tints_{n}")["tint_colours"]
        ):
            # Multiply with alpha does not work as you would expect - it just lowers the alpha of the
            # entire surface. To get around this, we first blit the tint onto a white background to dull it,
            # then blit the surface onto the sprite with pygame.BLEND_RGB_MULT
            tint = pygame.Surface((sprites.size, sprites.size)).convert_alpha()
            tint.fill(tuple(getattr(sprites, f"cat_tints_{n}")["tint_colours"][cat.pelt.tint]))
            new_sprite.blit(tint, (0, 0), special_flags=pygame.BLEND_RGB_MULT)
        if (
            cat.pelt.tint is not None
            and cat.pelt.tint in getattr(sprites, f"cat_tints_{n}")["dilute_tint_colours"]
        ):
            tint = pygame.Surface((sprites.size, sprites.size)).convert_alpha()
            tint.fill(tuple(getattr(sprites, f"cat_tints_{n}")["dilute_tint_colours"][cat.pelt.tint]))
        ...
        if cat.pelt.white_patches is not None:
            ...
            if patch in getattr(Pelt, f"mostly_white_{n}") or patch == "FULLWHITE":
                spritesheet = getattr(sprites, f"WHITE_MOSTLY_DATA_{n}")['spritesheet']
            elif patch in getattr(Pelt, f"high_white_{n}"):
                spritesheet = getattr(sprites, f"WHITE_HIGH_DATA_{n}")['spritesheet']
            elif patch in getattr(Pelt, f"mid_white_{n}"):
                spritesheet = getattr(sprites, f"WHITE_MID_DATA_{n}")['spritesheet']
            else:
                spritesheet = getattr(sprites, f"WHITE_LITTLE_DATA_{n}")['spritesheet']

            sprite_name = f"{spritesheet}{n}_{patch}{cat_sprite}"
            ...
            if (
                ...
                in getattr(sprites, f"white_patches_tints_{n}")["tint_colours"]
            ):
                tint = pygame.Surface((sprites.size, sprites.size)).convert_alpha()
                tint.fill(
                    tuple(
                        getattr(sprites, f"white_patches_tints_{n}")["tint_colours"][
        ...
        if cat.pelt.points:
            sprite_name = f"{getattr(sprites, f"WHITE_POINTS_DATA_{n}")['spritesheet']}{n}_{cat.pelt.points}{cat_sprite}"
        ...
        if cat.pelt.vitiligo:
            sprite_name = f"{getattr(sprites, f"WHITE_VITILIGO_DATA_{n}")['spritesheet']}{n}_{cat.pelt.vitiligo}{cat_sprite}"
        ...
        # draw eyes & scars1
        sprite_name = (
            f"{getattr(sprites, f"EYE_DATA_{n}")['spritesheet'][0]}{n}_{cat.pelt.eye_colour}{cat_sprite}"
        )
        ...
        if not scars_hidden:
            for scar in cat.pelt.scars:
                if scar in cat.pelt.general_scars:
                    sprite_name = (
                        f"{sprites.SCAR_DATA['spritesheet']}{n}_{scar}{cat_sprite}"
                    )
                    new_sprite.blit(
                        sprites.sprites[sprite_name],
                        (0, 0),
        ...
        gradient_surface = (
            sprites.sprites["line_ur_gradient" + f"{n}_" + cat_sprite]
        ...
        if game_setting_get("shaders") and not dead:
            ...
            new_sprite.blit(sprites.sprites["shader_lighting" + f"{n}_" + cat_sprite], (0, 0))

        if not dead:
            new_sprite.blit(sprites.sprites["lineart" + f"{n}_" + cat_sprite], (0, 0))
        elif cat.status.group == CatGroup.UNKNOWN_RESIDENCE:
            new_sprite.blit(sprites.sprites["lineart_ur" + f"{n}_" + cat_sprite], (0, 0))
        elif cat.status.group == CatGroup.DARK_FOREST:
            new_sprite.blit(sprites.sprites["lineart_df" + f"{n}_" + cat_sprite], (0, 0))
        elif dead:
            new_sprite.blit(sprites.sprites["lineart_sc" + f"{n}_" + cat_sprite], (0, 0))
        # draw skin and scars2
        blendmode = pygame.BLEND_RGBA_MIN
        sprite_name = f"{getattr(sprites, f"SKIN_DATA_{n}")['spritesheet']}{n}_{cat.pelt.skin}{cat_sprite}"
        new_sprite.blit(
            sprites.sprites[sprite_name],
            (0, 0),
        )
        ...
        if not scars_hidden:
            for scar in cat.pelt.scars:
                if scar in cat.pelt.missing_part_scars:
                    sprite_name = f"{sprites.SCAR_MISSING_PART_DATA['spritesheet']}{scar}{n}_{cat_sprite}"
                    new_sprite.blit(
                        _recolor_lineart(
                            sprites.sprites[sprite_name],
                            lineart_color,
        ...
        if (
            ...
            new_sprite.blit(
                sprites.sprites["fademask" + stage + f"{n}_" + cat_sprite],
                (0, 0),
                special_flags=pygame.BLEND_RGBA_MULT,
            )

            if cat.status.group == CatGroup.STARCLAN:
                temp = sprites.sprites["fadestarclan" + stage + f"{n}_" + cat_sprite].copy()
                temp.blit(new_sprite, (0, 0))
                new_sprite = temp
            elif cat.status.group == CatGroup.UNKNOWN_RESIDENCE:
                temp = sprites.sprites["fadeur" + stage + f"{n}_" + cat_sprite].copy()
                temp.blit(new_sprite, (0, 0))
                new_sprite = temp
            else:
                temp = sprites.sprites["fadedf" + stage + f"{n}_" + cat_sprite].copy()
                temp.blit(new_sprite, (0, 0))
                new_sprite = temp
        ...
        if dead:
            ...
            if cat.status.group == CatGroup.STARCLAN:
                ...
                # overlay
                temp_sprite.blit(
                    sprites.sprites["line_sc_overlay" + f"{n}_" + cat_sprite],
                    (0, 0),
                )
            elif cat.status.group == CatGroup.UNKNOWN_RESIDENCE:
                # underlay
                temp_sprite.blit(
                    sprites.sprites["line_ur_underlay" + f"{n}_" + cat_sprite],
                    (0, 0),
                )
                ...
                # overlay
                temp_sprite.blit(
                    sprites.sprites["line_ur_overlay" + f"{n}_" + cat_sprite],
                    (0, 0),
                )
            elif cat.status.group == CatGroup.DARK_FOREST:
                # no underlay
